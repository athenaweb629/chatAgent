...
const PEER_CONNECTION_EVENTS = ['connectionstatechange', 'icecandidate', 'icecandidateerror', 'iceconnectionstatechange', 'icegatheringstatechange', 'negotiationneeded', 'signalingstatechange', 'datachannel', 'track', 'error'];
...
class RTCPeerConnection extends (0, _eventTargetShim.defineCustomEventTarget)(...PEER_CONNECTION_EVENTS) {
  ...
  async setLocalDescription(sessionDescription) {
    ...
    this._updateTransceivers(transceiversInfo,
    /* removeStopped */
    ((_desc = desc) === null || _desc === void 0 ? void 0 : _desc.type) === 'answer');
  ...
  async setRemoteDescription(sessionDescription) {
    ...
    newTransceivers === null || newTransceivers === void 0 ? void 0 : newTransceivers.forEach(t => {
      ...
      const newSender = new _RTCRtpSender.default({ ...transceiver.sender,
        track: null
      });
      const remoteTrack = transceiver.receiver.track ? new _MediaStreamTrack.default(transceiver.receiver.track) : null;
      const newReceiver = new _RTCRtpReceiver.default({ ...transceiver.receiver,
        track: remoteTrack
      });
      const newTransceiver = new _RTCRtpTransceiver.default({ ...transceiver,
        sender: newSender,
    ...
    this._updateTransceivers(transceiversInfo,
    /* removeStopped */
    desc.type === 'answer'); // Fire track events. They must fire before sRD resolves.
    ...
    for (const ev of pendingTrackEvents) {
      const [transceiver] = this.getTransceivers().filter(t => t.receiver.id === ev.receiver.id); // We need to fire this event for an existing track sometimes, like
      ...
      transceiver._direction = ev.transceiver.direction; // Get the stream object from the event. Create if necessary.
  ...
  addTrack(track) {
    ...
    const {
      ...
      transceiver,
      sender
    } = result; // According to the W3C docs, the sender could have been reused, and
    ...
    if (existingSender) {
      // Update sender
      existingSender._track = track; // Update the corresponding transceiver as well
  ...
  addTransceiver(source, init) {
    ...
    const result = WebRTCModule.peerConnectionAddTransceiver(this._pcId, { ...src,
      init: { ...init
      }
    });
    ...
    const sender = new _RTCRtpSender.default({ ...t.sender,
      track
    });
    const remoteTrack = t.receiver.track ? new _MediaStreamTrack.default(t.receiver.track) : null;
    const receiver = new _RTCRtpReceiver.default({ ...t.receiver,
      track: remoteTrack
    });
    const transceiver = new _RTCRtpTransceiver.default({ ...result.transceiver,
      sender,
      receiver
    });
  ...
  close() {
    ...
    WebRTCModule.peerConnectionClose(this._pcId); // Mark transceivers as stopped.
  ...
  _registerEvents() {
    ...
    (0, _EventEmitter.addListener)(this, 'peerConnectionOnTrack', ev => {
      ...
      log.debug(`${this._pcId} ontrack`); // NOTE: We need to make sure the track event fires right before sRD completes,
      ...
      if (receiver && track) {
        ...
        for (const stream of this._remoteStreams.values()) {
          if (stream._tracks.includes(track)) {
            ...
            stream._tracks.splice(trackIdx, 1); // @ts-ignore
    ...
    (0, _EventEmitter.addListener)(this, 'peerConnectionGotICECandidate', ev => {
      ...
      const sdpInfo = ev.sdp; // Can happen when doing a rollback.
