...
export default class RTCPeerConnection extends defineCustomEventTarget(...PEER_CONNECTION_EVENTS) {
  ...
  addTransceiver(source, init) {
    ...
    const sender = new RTCRtpSender({ ...t.sender,
      track
    });
    const remoteTrack = t.receiver.track ? new MediaStreamTrack(t.receiver.track) : null;
    const receiver = new RTCRtpReceiver({ ...t.receiver,
      track: remoteTrack
    });
  ...
  _registerEvents() {
    ...
    addListener(this, 'peerConnectionGotICECandidate', ev => {
      ...
      const candidate = new RTCIceCandidate(ev.candidate); // @ts-ignore
    ...
    addListener(this, 'peerConnectionDidOpenDataChannel', ev => {
      ...
      const channel = new RTCDataChannel(ev.dataChannel); // @ts-ignore
