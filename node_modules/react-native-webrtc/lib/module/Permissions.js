...
class Permissions {
  ...
  _requestPermissionAndroid(perm) {
    return new Promise(resolve => {
      PermissionsAndroid.request(perm).then(granted => resolve(granted === PermissionsAndroid.RESULTS.GRANTED), () => resolve(false));
    });
  }
  /**
  ...
  query(permissionDesc) {
    ...
    if (Platform.OS === 'android') {
      const perm = permissionDesc.name === 'camera' ? PermissionsAndroid.PERMISSIONS.CAMERA : PermissionsAndroid.PERMISSIONS.RECORD_AUDIO;
      return new Promise(resolve => {
